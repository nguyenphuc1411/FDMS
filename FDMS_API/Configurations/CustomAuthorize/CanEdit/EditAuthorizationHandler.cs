...
{
    public class EditAuthorizationHandler:AuthorizationHandler<EditRequirement>
    {
        private readonly AppDbContext _dbContext;

        public EditAuthorizationHandler(AppDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, EditRequirement requirement)
        {
            ...
            // Lấy documentID từ Form
            if (!httpContext.Request.Form.TryGetValue("documentID", out var documentID))
            {
                context.Fail();
                return;
            }
            ...
            if (!string.IsNullOrEmpty(documentID.ToString()))
            {
                ...
                foreach (var group in userInGroups)
                {
                    if (groupCanAccessDocuments.Contains(group))
                    {
                        ...
                        if (permission != null && permission.CanModify)
                        {
